...
from refactored.source.trainer import train_model, generate_and_save_report
...
class TestTrainer:
    ...
    @patch('refactored.source.trainer.optuna')
    def test_train_model(self, mock_optuna, test_data, temp_dir):
        ...
        with patch('refactored.source.trainer.LGBMClassifier') as mock_lgbm:
                        ...
                        with patch.object(settings, 'BLOB_STUB_PATH', temp_dir):
                            with patch.object(settings, 'MODEL_NAME', 'test_model.pkl'):
                                with patch.object(settings, 'MODEL_DIR', ''):
                                    with patch('refactored.source.trainer.settings.FINAL_PARAMS_PATH', ''):
    ...
    def test_train_model_with_config(self, test_data, temp_dir):
        ...
        import refactored.source.trainer as trainer
                        ...
                        with patch('pickle.dump') as mock_pickle:
                            ...
                            with patch('os.makedirs') as mock_makedirs:
                                ...
                                with patch.object(settings, 'BLOB_STUB_PATH', temp_dir):
                                    with patch.object(settings, 'MODEL_NAME', 'test_model.pkl'):
                                        with patch.object(settings, 'MODEL_DIR', ''):
                                            with patch.object(settings, 'FINAL_PARAMS_PATH', ''):
