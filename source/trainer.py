...
def train_model(X_train: pd.DataFrame, y_train: pd.DataFrame, model_path: str) -> LGBMClassifier:
    ...
    params_file = os.path.join(settings.BLOB_STUB_PATH, settings.FINAL_PARAMS_PATH, 'best_params.txt')
...
def generate_and_save_report(best_model: LGBMClassifier, X_test: pd.DataFrame, y_test: pd.DataFrame, report_path: str = os.path.join(settings.BLOB_STUB_PATH, settings.REPORT_PATH)) -> None:
